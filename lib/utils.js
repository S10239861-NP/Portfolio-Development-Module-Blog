...
/**
 * 
 * @param {string} relativePathFromRootDir 
 */
export function redirectTo(relativePathFromRootDir)
{
    if (relativePathFromRootDir[0] == '.')
    {
        relativePathFromRootDir = relativePathFromRootDir.slice(1);
    }

    if (window.location.href.includes("localhost") == false)
    {
        relativePathFromRootDir = "./Portfolio-Development-Module-Blog" + relativePathFromRootDir;
    }

    window.location.href = relativePathFromRootDir;
}
